...
### preplanned telemetry vs ad hoc telemetry (hands on system) 
...
The pilot can only monitor what was preplanned to be monitor-able.

In software you can add some ad hoc telemetry with code changes followed by redeployments.
...
You transition from monitoring to troubleshooting when something is not working as you expect and you need it.
...
In the software world, the policies in practice (a.k.a the people) mostly seem to think that troubleshooting can be done with monitoring tools.
NUMBER 2) In my experience, IT/Infrastructure policies don't differentiate between monitoring and troubleshooting.

In the F-16 world, the maintenance team does has quite a few devices that provided preplanned telemetry for troubleshooting.
...
This is all "preplanned telemetry" because the maintenance team does not decide what to assess, rather the aircraft designers did.
...
But in the software world, I've never come across such a troubleshooting decision (that makes use of preplanned telemetry). 
...
#### transition preplanned telemetry to ad hoc telemetry
...
When I am doing anything with software I don't think about whether I am monitoring or troubleshooting.
This applies to development as well as sustainment because development consists of cycles of adding something and immediately troubleshooting it as it doesn't do what you expected it to do.
So I expect to be able to intermix monitoring flavored tools with troubleshooting flavored tools.
...
Some that do appreciate the difference just find it too inconvenient to accommodate "hands on system" troubleshooting access so they punt -- hoping that life will find a way with the already deployed "preplanned telemetry".


And life does _eventually_ find a way.
Maybe an expensive contractor like me gets hired to request elevated access and diagnose the problem.
